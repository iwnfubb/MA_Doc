...
Das Hintergrundsubtraktionsverfahren ist eine häufig verwendete Technik, die statische Kameras verwendet, um ein sich bewegtes Objekt aus dem Hintergrund zu extrahieren. Die Hintergrundsubtraktion erzeugt eine Binärbild, das die Pixel enthält, die zu den bewegenden Objekten in der Szene gehören. Wie der Name andeutet, wird die Vordergrundmaske durch die Hintergrundsubtraktion berechnet, was eine absolute Subtraktion zwischen dem aktuellen Bild und einem Hintergrundmodell darstellt. Der statische Teil der Szene oder allgemeiner alles, was angesichts der Merkmale der beobachteten Szene ist der der als Hintergrund klassifiziert werden kann, enthalten ist. Die Abbildung \ref{fig:BS_Example} stellt ein einfaches Beispiel für die Hintergrundsubtraktion dar. Das Ergebnis einer absoluten Subtraktion von dem aktuellen Bild und einem Hintergrundmodell ist ein Binärbild, indem die Bewegung einer Person mit weißen Pixel dargestellt wird.\\
...
\subsection{Adaptive Gaußschen Mixture Modell (AGMM)}
...
Unter Verwendung von der Hintergrundsubtraktion zusammen mit einer Histogrammanalyse wird eine bewegte Person erkannt bzw. auch die Körperhaltung ermittelt. Um aus diesem gewonnen Informationen eine außergewöhnliche Situationen zu erkennen, kommt die Fuzzylogik zum Einsatz. Im Jahre 1971 wurde die erste Forschung über die Fuzzy-Algebra veröffentlicht\cite{rosenfeld1971fuzzy}.\\
...
Bei der Fuzzylogik geht es um viele frei wählbaren Mengen, die jeweils einen Eingabeparameter abbilden. Die Mengen sind unter der gewählten Fuzzylogik definierten Regeln kombinierbar. Durch die verschiedenen Kombinationen entstehen neue Mengen, die Fuzzymengen genannt werden.
...
$A' = F(x_0)$\\
wobei $x_0$ ein scharfer Wert der Eingabe ist. $A'$ ist die Fuzzymenge mit Mitgliederfunktion $F$. $F$ kann eine beliebige stetig Funktion wie z.B.: Eine Sinus-, Kosinus-, Sigmoid-, Gaußschen-, glockenförmige Funktion sein. 
...
\end{itemize}
Die Abbildung \ref{fig:Fuzzy_Example} stellt diese drei Schritte der Fuzzylogik in graphischen Bildern dar. Beispielweise gibt es ein Smart-Thermostat in einem Raum und der Thermostat soll sich an der Raumtemperatur anpassen. Die hierzu erstellte Fuzzylogik bekommt die Temperatur als Eingabe und den Öffnungsgrad des Thermotatventils wird die Ausgabe (siehe Abbildung \ref{fig:Fuzzy_Example}). Der Regel-Algorithmus wird wie in Algorithmus \ref{algo:fuzzy} definiert. Die Temperatur wird als \glqq{}Low, Medium und High\grqq{} klassifiziert. Wenn die Temperatur von $0$ bis $20$ Grad ist, wird sie als \glqq{}Low\grqq{} klassifiziert. Werte zwischen $10$ und $20$ Grad werden als \glqq{}Medium\grqq{} klassifiziert. Der Bereich von $20$ bis $40$ Grad wird die Klassifizierung \glqq{}High\grqq{} zugeordnet. Bei Öffnungsgrad des Thermotatventils wird das geschlossene Intervall $[1, 2]$ als \glqq{}Low\grqq{}, das Intervall $[1, 4]$ als \glqq{}Medium\grqq{} und letztlich das Intervall $[3, 5]$ als \glqq{}High\grqq{} klassifiziert. Eine Beispieleingabe der Temperatur von $23$ Grad, die sich zwischen \glqq{}Medium\grqq{} und \glqq{}High\grqq{} befindet, ist in Abbildung \ref{fig:Fuzzy_Example} zu sehen. Bei $23$ Grad beträgt der Mitgliedschaftsgrad $0.7$ zur Menge \glqq{}Medium\grqq{} und $0.3$ zu \glqq{}High\grqq{}. Nach der Regel in Algorithmus \ref{algo:fuzzy}, wird der Öffnungsgrad des Thermotatventils zwischen \glqq{}Low\grqq{} und \glqq{}Medium\grqq{} berechnet und mit der Methode \glqq{}Schwerpunkt\grqq{} liegt der Öffnungsgrad des Thermotatventils bei $2.15$.\\
\begin{figure}[H]
	\centering
...
OpenCV steht für Open-Computer-Vision. Die Bibliothek ist in C und C++ programmiert und ist Betriebssystem unabhängig. OpenCV kann auch mithilfe von Java, Python, Ruby, Mathlab... verwendet werden. OpenCV wurde für mit Fokus auf Recheneffizienz entwickelt, was für Echtzeitanwendungen relevant ist. Diese Bibliothek enthält über 500 Funktionen, die viele Bereiche in der Bildverarbeitung einschließlich maschinelles Lernen, neuronale Netze, usw. umfassen\cite{bradski2008learning}. OpenCV vereinfacht den Bildverarbeitungsprozess mit vielen hilfreichen vordefinierten Funktionen. Da es in dieser Arbeit sich um ein Echtzeitanwendung handelt, wird auf OpenCV gesetzt, um die Verarbeitungszeit gering zu halten.
\section{360° Kamera}
Ziel dieser Arbeit ist es, die Entwicklung einer Echtzeitanwendung, die auch mit der von Bosch entwickelte Innenkamera funktioniert. Die Testvideos in dieser Arbeit wurden mit einer Bosch Innenkamera aufgenommen. Die Bosch 360° Innenkamera kann sich in jede Richtung (360°) drehen. Die Kamera verfügt über Bewegungssensoren, eine Gegensprechanlage für Zwei-Wege-Audio und eine Infrarot-Nachtsicht. Sie zeichnet Bild-Material in HD auf. Das Bild-Material kann auf PC bzw. externen Speicher kopiert werden. Die Innenkamera verfügt über verschiedene Schnittstellen wie zum Beispiel WLAN. Die Internetverbindung wird von der Kamera genutzt, um Push-Nachrichten an ein Smartphone schicken zu schicken. Dank der integrierten Bewegungsmeldern kann die Kamera eine Bewegung folgen bzw. sich automatisch drehen, um die das bewegte Objekt im Bild zu halten. Dies Feature ist in dieser Arbeit sehr hilfreich. Die Kamera hat dazu noch zwei besondere Fähigkeiten, sich in jede Richtung zu drehen und mit einer Infrarot-Nachtsicht aufzunehmen. So bleibt die Kamera auch abends bzw. wo kein Licht ist voll funktionsfähig. 
\begin{figure}[H]
	\centering
